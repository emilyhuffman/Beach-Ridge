...
def swash_wash_params(Qri,Qs,Tmax,dt,m,a,b,maxH):
    ...
    # if ridges can nucleate
        if suppress==0: #no special conditons limiting ridge growth
            ...
            if (SeawardFlankToe - Shoreline > maxdis) or (H > maxH+2): 
                ...
                flankcrest.append(-InlandFlankCrest)
    ...
    filtered_H= [item for item in Hst if item is not None]

    return np.diff(flankcrest), filtered_H, RidgeCounter, time_for_ridge, Dst
